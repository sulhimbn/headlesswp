      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <ul className="space-y-2">
                  {footerLinks.map((link, index) => (
                    <li key={`${link.href}-${index}`}>
                      <Link
                        href={link.href}
                        className="hover:text-[hsl(var(--color-primary))] transition-all duration-[var(--transition-fast)] focus:outline-none focus:ring-2 focus:ring-[hsl(var(--color-primary))] focus:ring-offset-2 rounded-[var(--radius-sm)]"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="border-t pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
            <p className="text-sm text-center md:text-left" dangerouslySetInnerHTML={{ __html: UI_TEXT.footer.copyright(currentYear) }} />
