...
async function handler(request: Request) {
  try {
    ...
    const perPage = Math.min(parseInt(searchParams.get('per_page') || '10', 10), MAX_PER_PAGE)
    const page = Math.min(parseInt(searchParams.get('page') || '1', 10), MAX_PAGE)
...
export const GET = withApiRateLimit(handler, 'metrics')
