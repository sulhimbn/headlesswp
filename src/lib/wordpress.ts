...
export class WordPressAPI implements IWordPressAPI {
  ...
  async getPostById(id: number, signal?: AbortSignal): Promise<WordPressPost> {
  ...
  async getMedia(id: number, signal?: AbortSignal): Promise<WordPressMedia> {
    const response = await apiClient.get(getApiUrl(`/wp/v2/media/${id}`), { signal });
    return response.data;
  }

  async getMediaUrl(mediaId: number, signal?: AbortSignal): Promise<string | null> {
  ...
  async getMediaBatch(ids: number[], signal?: AbortSignal): Promise<Map<number, WordPressMedia>> {
  ...
  async getMediaUrlsBatch(mediaIds: number[], signal?: AbortSignal): Promise<Map<number, string | null>> {
  ...
  async getAuthor(id: number, signal?: AbortSignal): Promise<WordPressAuthor> {
    const response = await apiClient.get(getApiUrl(`/wp/v2/users/${id}`), { signal });
    return response.data;
  }

  async search(query: string, page: number = 1, perPage: number = 12, signal?: AbortSignal): Promise<{ posts: WordPressPost[], totalPages: number }> {
...
export const wordpressAPI: IWordPressAPI = new WordPressAPI();
