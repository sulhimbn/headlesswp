...
jest.mock("@/lib/utils/readingHistory", () => ({
  ...
  getReadingHistory: jest.fn(() => {
    const stored = (global.localStorage?.getItem &&
      JSON.parse(
        global.localStorage.getItem("reading_history") || '{"items":[]}',
      )) || { items: [] };
    return {
      items: stored.items || [],
...
describe("PersonalizedRecommendations Component", () => {
  ...
  describe("Loading state", () => {
    test("renders loading skeleton", async () => {
      ...
      expect(
        screen.getByRole("heading", { name: "Rekomendasi Untuk Anda" }),
      ).toBeInTheDocument();
    });
  });
    ...
    test("handles media fetch gracefully", async () => {
      ...
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({ recommendations: postsWithMedia }),
        })
        .mockResolvedValueOnce({
          ok: false,
        });
